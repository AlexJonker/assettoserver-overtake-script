...
function script.drawUI()
    ...
    -- Score cards - redesigned layout
    ui.offsetCursorY(15)
    
    -- Current score card (full width)
    local cardHeight = 80
    ui.drawRectFilled(vec2(0, ui.getCursorY()), vec2(windowSize.x, ui.getCursorY() + cardHeight), colorCard, 8)
    ui.drawRect(vec2(0, ui.getCursorY()), vec2(windowSize.x, ui.getCursorY() + cardHeight), rgbm(0.3, 0.3, 0.35, 0.5), 1, 8)
    
    -- Center current score vertically
    ui.setCursor(vec2(0, ui.getCursorY() + (cardHeight/2 - 25)))
    ui.pushFont(ui.Font.Small)
    ui.textColored("CURRENT SCORE", rgbm(0.7, 0.7, 0.7, 1))
    ui.popFont()
    
    ui.setCursor(vec2(0, ui.getCursorY() + 5))
    ui.pushFont(ui.Font.Huge)
    ui.textColored(string.format("%06d", totalScore), colorText)
    ui.popFont()
    
    -- High score directly underneath (bigger font)
    ui.setCursor(vec2(0, ui.getCursorY() + 30))
    ui.pushFont(ui.Font.Main)  -- Bigger than Small but smaller than Huge
    ui.textColored("HIGH SCORE: " .. string.format("%06d", highestScore), rgbm(0.7, 0.7, 0.7, 1))
    ui.popFont()
    
    -- Combo card (right side, below scores)
    ui.offsetCursorY(90)
    ui.drawRectFilled(vec2(0, ui.getCursorY()), vec2(windowSize.x, 60), colorCard, 8)
    ui.drawRect(vec2(0, ui.getCursorY()), vec2(windowSize.x, 60), rgbm(0.3, 0.3, 0.35, 0.5), 1, 8)
    
    -- Center combo vertically
    ui.setCursor(vec2(0, ui.getCursorY() + 10))
    ui.pushFont(ui.Font.Small)
    ...
    -- Speed warning (fixed position below combo)
    ui.offsetCursorY(70)
    if speedWarning > 0.1 then
        local warningHeight = 40
        ui.drawRectFilled(vec2(0, ui.getCursorY()), vec2(windowSize.x, ui.getCursorY() + warningHeight), rgbm(0.15, 0.1, 0.05, 0.7), 6)
        ui.drawRect(vec2(0, ui.getCursorY()), vec2(windowSize.x, ui.getCursorY() + warningHeight), rgbm(0.8, 0.4, 0.1, 0.5), 1, 6)
        ...
        -- Speed progress bar (fixed position)
        local progress = math.min(player.speedKmh/requiredSpeed, 1)
        local barWidth = windowSize.x - 20
        ui.drawRectFilled(vec2(10, ui.getCursorY() + 25), vec2(10 + barWidth, ui.getCursorY() + 30), rgbm(0.2, 0.2, 0.2, 1), 3)
        ui.drawRectFilled(vec2(10, ui.getCursorY() + 25), vec2(10 + barWidth * progress, ui.getCursorY() + 30), colorWarning, 3)
        
        -- Speed text at fixed position (not overlapping)
        ui.drawText(vec2(windowSize.x/2 - 30, ui.getCursorY() + 32), 
                   string.format("%d/%d km/h", math.floor(player.speedKmh), requiredSpeed), 
                   rgbm(0.9, 0.9, 0.9, 1))
    end
    
    -- Messages area (fixed position at bottom)
    ui.offsetCursorY(50)
    local messagesHeight = 60
